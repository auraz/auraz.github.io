(function(reguirejs, require, define){
    define(['jquery', 'plot/primitives/point'], function($, Point){
...
var Curve = function(from, to, cFr, cTo, state){
    ...
    Curve.prototype = {
        ...
        'getInitialState': function(){
            var self = this,
                i = 0,
                l = this.pathArray[1].length,
                X = self.pathArray[0][1] - self.initialState[0][1],
                Y = self.pathArray[0][2] - self.initialState[0][2];

            this.pathArray[0][1] = this.initialState[0][1];
            this.pathArray[0][2] = this.initialState[0][2];
    
            for ( i = 1; i < l; i += 2 ){
                this.pathArray[1][i] = this.initialState[1][i]; 
            }
        ...
        'updateCoefs': function(coords, shift){
            var controls = this.controls,
                ...
                this.pathArray[1][1] = from.x;
                this.pathArray[1][2] = from.y;
                
                this.pathArray[1][3] = to.x;
                this.pathArray[1][4] = to.y;
                this.path.attr({
                    path: this.pathArray
                });
        },
        'interpolate': function(flag){             
            ...
            if(flag) {
                this.updateCoefs({
                    'x': this.pathArray[1][5],
                    'y': this.pathArray[1][6]
                }, {
                    'x': this.pathArray[0][1],
                    'y': this.pathArray[0][2]
                });                
            } else {
                this.updateCoefs({
                    'x': BB.MAX_X_VALUE,
